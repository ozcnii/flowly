...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (habit.status === "active") return NextResponse.json({ habit, idempotent: true });
  const updatedAt = nowIso();
  await db.update(schema.habits).set({ status: "active", updatedAt }).where(and(eq(schema.habits.id, id), eq(schema.habits.ownerId, userId), eq(schema.habits.status, "paused")));
  const updated = (await db.select().from(schema.habits).where(eq(schema.habits.id, id)).limit(1))[0];
  audit("habit.resume", { userId, habitId: id });
