...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await transitionHabitOccurrence(getDb(), userId, (await params).id, "completed", parsed.data ?? null, "habit_complete");
  if (result.kind === "ok") {
    audit("habit.occurrence.complete", { userId, occurrenceId: (await params).id, idempotent: result.idempotent });
