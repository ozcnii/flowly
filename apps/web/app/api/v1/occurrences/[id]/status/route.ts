...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = occurrenceStatusMutationSchema.safeParse(await request.json().catch(() => ({})));
  if (!parsed.success) return NextResponse.json({ error: "invalid", issues: parsed.error.issues }, { status: 400 });
  const result = await transitionHabitOccurrence(getDb(), userId, (await params).id, parsed.data.status, parsed.data.comment ?? null, "habit_manual");
  if (result.kind === "ok") {
    audit("habit.occurrence.status", { userId, occurrenceId: (await params).id, status: result.occurrence.status, idempotent: result.idempotent });
