...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await transitionHabitOccurrence(getDb(), userId, (await params).id, "rest", parsed.data ?? null, "habit_rest");
  if (result.kind === "ok") {
    audit("habit.occurrence.rest", { userId, occurrenceId: (await params).id, idempotent: result.idempotent });
