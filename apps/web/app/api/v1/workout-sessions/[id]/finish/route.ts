...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const occurrenceId = generateId(), historyId = generateId(), ts = nowIso(), local = localDateTime(new Date(ts), user.timezone), raw = getD1();
  ...
  audit("workout_session.finish", { userId, sessionId: id, occurrenceId, finalStatus: parsed.data.finalStatus });
