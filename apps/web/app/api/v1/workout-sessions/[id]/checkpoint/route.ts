...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ts = nowIso(), changed = await db.update(schema.workoutSessions).set({ accumulatedSeconds: parsed.data.accumulatedSeconds, playbackPositionSeconds: parsed.data.playbackPositionSeconds ?? current.playbackPositionSeconds, pausedAt: parsed.data.paused ? ts : null, updatedAt: ts }).where(and(eq(schema.workoutSessions.id, id), eq(schema.workoutSessions.userId, userId), eq(schema.workoutSessions.state, "open"), eq(schema.workoutSessions.updatedAt, parsed.data.baseUpdatedAt))).returning({ id: schema.workoutSessions.id });
