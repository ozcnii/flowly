...
export async function POST(request: Request) {
  ...
  if (!target.executable) return NextResponse.json({ error: "not_executable", message: "Для этой тренировки пока нет исполняемого содержания." }, { status: 409 });
  const mode = target.workout.format === "video" ? "video" : parsed.data.mode ?? "step";
  const id = generateId(), ts = nowIso();
  try { await db.insert(schema.workoutSessions).values({ id, userId, workoutId: target.workout.id, occurrenceId: null, state: "open", mode, startedAt: ts, pausedAt: ts, accumulatedSeconds: 0, playbackPositionSeconds: 0, completedAt: null, finalStatus: null, currentExercisePosition: null, createdAt: ts, updatedAt: ts }); }
