    ...
    <>
      ...
      <main className="flow-screen pb-safe-4">
        ...
        <form className="contents" onSubmit={onSubmit} noValidate>
          ...
          <List strong inset dividers>
            {scheduleType === "exact_times" ? <>{scheduleTimes.map((time, index) => <ListInput key={`${time}-${index}`} title={`Время ${index + 1}`} type="time" value={time} className="cursor-pointer" onClick={(e) => { const input = e.currentTarget instanceof HTMLInputElement ? e.currentTarget : e.currentTarget.querySelector("input"); input?.showPicker?.(); }} onInput={(e) => setScheduleTimes((items) => items.map((item, i) => i === index ? (e.currentTarget as HTMLInputElement).value : item))} aria-label={`Время привычки ${index + 1}`} />)}<Button clear rounded type="button" onClick={() => setScheduleTimes((items) => [...items, "21:00"])}>Добавить время</Button></> : <>{scheduleDayTimes.map((time, index) => <ListInput key={`${time}-${index}`} title={`Время выбранных дней ${index + 1}`} type="time" value={time} className="cursor-pointer" onClick={(e) => { const input = e.currentTarget instanceof HTMLInputElement ? e.currentTarget : e.currentTarget.querySelector("input"); input?.showPicker?.(); }} onInput={(e) => setScheduleDayTimes((items) => items.map((item, i) => i === index ? (e.currentTarget as HTMLInputElement).value : item))} aria-label={`Время выбранных дней ${index + 1}`} />)}<Button clear rounded type="button" onClick={() => setScheduleDayTimes((items) => [...items, "21:00"])}>Добавить время</Button></>}
          </List>
