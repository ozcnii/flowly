              ...
              <List strong inset dividers>
                {(scheduleType === "exact_times" ? scheduleTimes : scheduleDayTimes).map((time, index, items) => (
                  <ListInput
                    ...
                    label={`Время выполнения ${index + 1}`}
                    title={`Время выполнения ${index + 1}`}
                    type="time"
                    value={time}
                    inputStyle={{ fontSize: 16, minHeight: 44 }}
          ...
          {scheduleType === "weekly_target" ? (
            <>
              ...
              <List strong inset dividers>
                ...
                <ListInput title="Предпочтительное время" type="time" value={weeklyTime} onInput={(e) => setWeeklyTime((e.currentTarget as HTMLInputElement).value)} aria-label="Предпочтительное время" inputStyle={{ fontSize: 16, minHeight: 44 }} />
              </List>
            </>
          ) : null}
          {scheduleType === "interval" ? (
            <>
              ...
              <List strong inset dividers>
                ...
                <ListInput title="Время точки отсчёта" type="time" value={intervalAnchorTime} onInput={(e) => setIntervalAnchorTime((e.currentTarget as HTMLInputElement).value)} aria-label="Время точки отсчёта" inputStyle={{ fontSize: 16, minHeight: 44 }} />
              </List>
