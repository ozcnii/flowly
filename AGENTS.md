...
- **Layout stability — layout не имеет права сдвигаться от transient states.** Появление/исчезновение loading/pending/error/sync/navigate-indicator НЕ может менять геометрию (высоту/позицию) окружающих элементов: например, кнопка, при нажатии добавляющая сверху loading-текст, а потом убирающая его, сдвигает интерфейс дважды — это запрещено. Допустимые приёмы: (1) зарезервировать место заранее (`min-h-*`/fixed slot для status/описания/таймера); (2) overlay поверх области с уже фиксированной высотой (cover aspect-video, absolute spinner); (3) унести transient-стейт на следующий экран (навигация со спиннером там). Перед review обязателен аудит каждого экрана на отсутствие layout-shift от transient states; нарушение — блокер.
...
- **Apple Design skill обязателен для UI/UX:** перед любым UI/UX analysis, implementation или review прочитать [`.pi/skills/apple-design/SKILL.md`](.pi/skills/apple-design/SKILL.md). Применять его motion, spatial consistency, response, reduced-motion, typography и restraint principles только через существующие direct Konsta components/shared Flowly contracts; raw HTML/CSS, custom motion libraries, custom controls и дублирующие Konsta surfaces из исходного web skill не переносить.
