...
// Starter exercise art ships as webp stills under public/media/catalog/exercises (real motion GIFs later).
out.push(exercises.map((e) => tuple([sql(e.id), "NULL", sql(e.title), sql(e.description), sql(`catalog/exercises/${e.id}.webp`), sql(e.mediaType === "gif" ? "image" : e.mediaType), e.duration ?? "NULL", e.repetitions ?? "NULL", sql(ts), sql(ts)])).join(",\n") + "\nON CONFLICT(id) DO UPDATE SET title=excluded.title, description=excluded.description, media_object_key=excluded.media_object_key, media_type=excluded.media_type, default_duration_seconds=excluded.default_duration_seconds, default_repetitions=excluded.default_repetitions, updated_at=excluded.updated_at;\n");
