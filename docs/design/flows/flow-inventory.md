...
- Persistent navigation has exactly five tabs with 9px labels (DEC-043). On every exact top-level tab route shared Konsta Navbar shows Settings on the left and a constant `user-round` Profile action on the right; Home title is the Flowly name, other titles match tab labels (DEC-043).
