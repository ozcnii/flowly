...
- Inventory содержит 68 стабильных screen/surface IDs после удаления S-MA-096 по DEC-041 и 11 flow IDs (`F01`–`F11`).
- Canonical membership задаётся `Screens` в [`flow-inventory.md`](flow-inventory.md), зеркально совпадает с `Flow` каждого screen record и проверена сравнением множеств: 96 пар с каждой стороны, symmetric difference пуст.
...
- Состояния больше не назначаются blanket-диапазонами: все 68 IDs имеют отдельное назначение одного из 15 определённых profiles; read-only collection/detail/report profiles отдельно фиксируют `saved = N/A`, каждый `N/A` содержит причину, а строка ID уточняет surface-specific outcome.
...
|---|---|---|
| Exact IDs + bidirectional sets | Python parse inventory rows, expand ranges, compare `(screen, flow)` sets | PASS: 68 screens, F01–F11, 96 pairs each side, symmetric difference `[]` |
...
| Per-ID states | Python parse explicit ID→profile assignments and profile definitions | PASS: 68/68 unique assignments, 15 defined profiles |
...
```text
IDs PASS: 68 screens, 11 flows; bidirectional pairs PASS: 96
States PASS: 68 explicit per-ID assignments across 15 defined profiles
...
3. All 13 reported asymmetric relations were reconciled; the resulting current set has 96 symmetric pairs after DEC-041.
