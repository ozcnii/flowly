...
- [ ] DEC-061 route matrix: пять exact roots + `/catalog?...` имеют один Tabbar/`pb-safe-24`; Profile/Settings/YouTube/Workout/Session/Sources/Safety имеют 0 Tabbar/`pb-safe-4`; Home→Profile→Settings→Back×2 и Catalog→internal→Back не remount/flicker и восстанавливают Tabbar только на root.
...
- [ ] Catalog/YouTube results используют один DEC-053 `@/components/workouts/workout-media-card` с two-line visual/full accessible title и compact 44px domain icon action; Search/YouTube-detail playback использует shared Popup, а raw iframe существует только в `@/components/youtube/youtube-iframe-player` и переиспользуется DEC-062 session runtime; external app Watch links и duplicate iframe/player implementations отсутствуют.
- [ ] DEC-059 exact Glass matrix: Profile/filter/play используют shared `GlassIconButton` 44×44 + icon 20×20 с native Konsta glass/highlight/pressed/focus; play есть в YouTube Search и YouTube detail, но отсутствует в Catalog cards; timezone Searchbar inset ровно 16px с обеих сторон.
- [ ] DEC-062 video session matrix: executable Start only with `youtubeVideoId`; one-open conflict has Continue/Close/Cancel; `/sessions/[id]` has Tabbar 0, IFrame API active-time, pause/resume, 15s server + per-second monotonic local + pagehide/unmount persistence, independent playback-position restore/seek, `<1s` auto-server without Sheet and `>=1s` explicit server/device conflict, ended without auto-finish, five statuses with `completed` selected by default on every open, final footer clears rounded iPhone edge via `pb-safe-6` and comment action cannot create X overflow at Dynamic Type, occurrence/history idempotency and compact Home resume with same-active direct Continue + unbounded `m:ss`/`h:mm:ss`; real Telegram autoplay/player events checked.
- [ ] DEC-063 YouTube result matrix: main card materializes/reuses one private workout and opens shared detail; Play only opens shared Popup; Save/Open race yields one ID; generated empty detail has no Exercises/Details gaps; seeded exercises survive; Finish occurrence joins to `source_type=youtube`; no duplicate detail/player/session/materialization logic.
