...
- **Текущий этап:** 2. Йога.
- **Активная задача:** E2-D2-T05 (review).
- **Статус:** E2-D2-T05 favorites `review` + UX fix after user repro. Visual on/off: outline `bookmark` vs filled `bookmark-fill` + tonal/accent; toggle remove works; optimistic list; YouTube card is favorite toggle (materialize+PUT / DELETE), not one-way check Save. `/favorites` lists favorited workouts (YouTube-only Save without favorite no longer pretends to be favorites). E2-D3-T03 `blocked` (DEC-064). Manual log **E2-D3-T04 → E6-D7-T09** (DEC-065). T02+T06 `done`. Historical: Migration `0009` adds `workout_sessions.mode`; step runtime extends DEC-062 `/sessions/[id]` with exercises/position checkpoint, auto exercise→rest→next (timer stays on after first Продолжить), +30 on countdown/rest, last-5s ticks, complete fanfare+confetti, last-step «К итогу»/✓, done return path, conflict parity with video (elapsed &lt;1s auto-server). Shared Finish/Conflict sheets extracted. Residual: S-MA-032 mixed chooser not shipped; exercise media placeholder until E2-D3-T03. Historical: E2-D2-T08 closed `done`. Реализация другой нейронки подтверждена корректной: shared `handleYoutubeWorkoutPost` materialization service (Save + create-workout без copy-paste), DB partial unique index 0008, card main-open → `/workouts/[id]`, Play остаётся DEC-053 popup без materialization, generated empty detail скрывает Exercises/Details и переиспользует общий Hero/ActionPanel/session, occurrence→workout join возвращает `source_type=youtube` (report-source data contract для E6). Re-verification PASS: concurrent real-video Save+Create → один ID, fresh video `created=true`, repeat `created=false` тот же ID; generated detail API `sourceType=youtube/format=video/exercises0/start.enabled=true`; browser `/youtube` 390px — ровно 3 действия на карточку (Открыть/Воспроизвести/Сохранить), 0 console errors, click «Открыть» → `/workouts/<id>` без reload; generated-detail DOM `hasExercises=false/hasDetails=false/Начать enabled/Hero Play visible/overflow=0`; UI идентичен user-reference `/workouts/019f6a1f-...`; Start→Finish создал occurrence, SQL join подтверждает YouTube source; root typecheck/lint/build PASS. Пользователь явно разрешил commit+push и закрытие задачи; commit pending. Historical T01: exact pre-fix 0:03→Back терял local/server progress; post-fix сохраняет/восстанавливает elapsed+playback, Pause geometry stable, Browser/native canonical Back PASS; unrelated-detail sequence не reproduced. Correction v3 explicitly approved and implemented: migration 0007 persists YouTube playback position separately from elapsed through checkpoint/finish/local snapshot; Player handle restores it through `seekTo`. Muted browser equal-elapsed stale-token case auto-selects server with no Sheet and seeks to position38; `>=1s` remains explicit conflict by contract. HTTP checkpoint/stale/idempotent-finish evidence in `.temp/E2-D3-T01/http/playback-*`; rolling old-client bodies preserve position44. Full typecheck/lint/build/OpenNext deploy-check/diff-check PASS. Production deployed: commit `b69e190` (`feat(web): add resumable video sessions`) pushed to `origin/main`; GitHub Actions run `29579333079` PASS, including typecheck, lint, remote D1 migrations 0006/0007 and web worker deploy. Browser-UA smoke: `/` 200, `/sessions/smoke` 200, unauthenticated active-session API 401. Initial generic Python UA hit expected Cloudflare 1010, browser UA passed. Latest iPhone follow-up deployed: final Sheet footer `pb-safe-4→pb-safe-6`, comment action bounded/wrappable, Sheet/content X-clamped. Muted 200% check shows document/body/Sheet overflow 0, bottom gap 48px at doubled root scale, console errors 0. Commit `85b9cc4`, production run `29582336030` PASS.
...
1. **User review E2-D2-T05** favorites (Catalog/Sources/Detail/Profile/Favorites toggle + filter). Then close `review -> done` if OK.
2. **Next pick (stage 2):** E2-D3-T05 DoD or residual S-MA-032 mixed — **not** manual log.
3. **E2-D3-T04 → E6-D7-T09** (DEC-065): manual workout log with calendar/diary, after E6-D7-T01/T02.
4. **E2-D3-T03 frozen** (DEC-064). Visible report UI Badge `YouTube` remains E6.
