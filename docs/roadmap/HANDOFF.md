...
- **Статус:** E0-D0-T05 остаётся `in_progress`; Konsta migration и route-level correction passes реализованы. Последние approved scopes — DEC-052 app-owned Telegram Back history, DEC-053 shared cards/player, DEC-054 workout detail, DEC-057 unified sources и DEC-060 media paint — browser/mock verified; typecheck/lint/production build/deploy-check PASS, нужны real-device Telegram Back/Close/X/player/autoplay/media-paint rerun и общий user review.
...
1. На реальном Telegram iOS/Android проверить Home→Yoga→Back, Settings→Tracks→Back, direct workout fallback, rapid Back, reload и Home-boundary closing confirmation; отдельно подтвердить неизбежный native Close/X confirmation contract и отсутствие icon/image flash при повторной навигации.
...
- `packages/ui/**`, `apps/web/app/ui-kit/**` — E1-D1-T11 production UI-kit; DEC-060 inline icon sprite and same-document icon fragments
