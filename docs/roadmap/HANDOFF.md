...
- **Активная задача:** none (next **E3-D4-T07**).
- **Статус:** T01–T06 `done`. T06: `ensureProgramOccurrences` on enroll/GET; unique index 0013; skip/rest update same row; finish reuses occ; planned rest = program_day+status rest.
...
1. **E3-D4-T07** — reminder_jobs for program occurrences (no Telegram send) + §55.3 DoD evidence.
2. Stage 3 closeout.
