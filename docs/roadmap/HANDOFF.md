...
- **Последний подтверждённый результат:** diagnostic production rerun доказал точную причину: `source=hash`, `webApp=0`, `hasSignature=true`, `clientFpMatches=true`, `detail=invalid hash`. Payload доходит без искажения; CSP блокирует SDK, а verifier ошибочно исключал `signature` из bot-token HMAC. Fix реализован и локально проверен, ожидает production deploy/rerun.
