...
- **Последняя закрытая задача:** E4-D5-T04 — недельная цель и интервальное расписание, `done` после подтверждения локальных проверок.
...
- **Следующее точное действие:** продолжать только после отдельного approval следующего screen slice; occurrences и S-MA-060 progress остаются T07 scope.
