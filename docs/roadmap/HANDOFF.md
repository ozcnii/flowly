...
- **Текущая задача:** E4-D5-T08 — закрыть DoD привычек, `in_progress`.
...
- **README sync:** активный этап 4; task counts синхронизированы: 45 done / 1 blocked / 31 backlog / 1 in_progress / 0 review.
...
- **T08 plan:** `.temp/E4-D5-T08/plan.md` подготовлен на русском; verification-first matrix покрывает четыре schedule type, multiple completions, privacy, skip/rest, icons/colors, DST и scheduler. Plan confidence 92%, implementation confidence 90%. Реализация/проверки ожидают user approval плана.
- **Следующее точное действие T08:** получить approval `.temp/E4-D5-T08/plan.md`, затем выполнить scenario matrix до внесения runtime-изменений. Перед production T07 остаются deploy/real-device scheduler checks и E5 Telegram delivery.
