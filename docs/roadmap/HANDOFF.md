...
- Catalog `/catalog` standalone code/browser/Apple HIG review 2026-07-15: gate **FAIL**, implementation not changed pending user decisions. Primary Navbar inset-0 fix `045e0a2` was rejected after iPhone showed title under notch; superseded by `429beb9`: desktop/web hides safe-area Navbar, Telegram iOS/Android uses `max(44px, composed top)` with title centered in final 44px; Deploy web run 29398306357 PASS; browser/real-device rerun pending. Major: forced `offline` disables React Query and remains banner + custom skeleton forever; Sheet does not move/trap focus, does not make background inert and ignores Escape; 200% text scaling clips card/search content despite overflow 0; filter selection is color-only and single-select groups use toggle-button semantics; card contract omits required source/category/format/author/UGC cues; 23 disabled favorite controls add non-action clutter. Strict Konsta audit: catalog still has a 480-line CSS Module, 30 classes/4 dead selectors, rewrites Searchbar/Card/Sheet/Button/focus visuals and implements raw custom Skeleton, contrary to DEC-035 cleanup trigger. Normal 360/390/430 light/dark: horizontal overflow 0, visible targets ≥44, console errors 0, last-card clearance ~32px. Evidence `.temp/E0-D0-T05/screenshots/catalog-review-*` and `.temp/E0-D0-T05/catalog-review-snapshot.md`. Blocking choices: filter control model and whether disabled favorite is removed, retained, or implemented.
