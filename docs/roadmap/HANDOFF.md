...
- **Текущий этап:** 1. Основа (profile/onboarding production wiring)
- **Активная задача:** E1-D1-T13 (Real profile + production onboarding) — `in_progress`.
- **Статус:** Telegram auth fix подтверждён production (`source=sdk`, `webApp=1`, `auth.login`, Home открыт). E1-D1-T12 переведена в `blocked` только до обязательной ротации ранее раскрытого token. Production screenshots выявили hardcoded identity на Home/Profile/Settings; пользователь утвердил новый scope profile + onboarding. E2-D2-T06 и E2-D2-T07 остаются в `review`.
- **Последний подтверждённый результат:** E1-D1-T13 реализована и локально approved пользователем («все работает»): real `/me` identity на Home/Profile/Settings, durable D1 onboarding marker, fullscreen production flow/guards, disabled honest habit/invite controls, explicit red mutation errors и отдельная HTTP-safe dev session cookie без ослабления production cookie. Пользователь попросил commit/push и будет тестировать production; задача остаётся `in_progress` до Telegram Desktop approval.

## Что сделано

- E1-D1-T12 production verification PASS: deploy `5b714a7`; Telegram Desktop SDK `macos/9.6`, signed initData validated, session created, Home visible. Остался внешний security blocker: revoke/regenerate token, Cloudflare secret update, webhook recheck.
- E1-D1-T13 implementation: DEC-034; `users.onboarding_completed_at` + migration 0004; `GET /me.onboardingCompleted`; idempotent session+Origin protected `POST /api/v1/onboarding/complete`; React Query completion mutation; product/onboarding guards; `/onboarding/[step]`; welcome/preferences/capabilities/bot sequence; real name/username/photo on Home/Profile/Settings; settings initializes from `/me`; S-MA-004 controls disabled with TODO(E4-D5-T02/E7-D8-T01); red `InlineError` distinguishes 401/offline/server failures. Local dev works without Telegram via separate `flowly-dev-session` over HTTP; production retains `__Host-flowly-session; Secure`. Checks: migration/API matrix, typecheck, targeted lint, build, fetch/hardcode audits, diff-check PASS; local user approval received. Commit `00d1069` pushed; GitHub Actions `29382146616` PASS (typecheck/lint/remote migration/deploy); production health `/` 200, `/onboarding/welcome` 200, unauthenticated `/api/v1/me` 401. Live `onboarding.*` tail запущен. Next: production Telegram flow review; затем evidence/approval и решение `in_progress -> review`.

- E2-D2-T07 shell/catalog follow-up готов локально по user feedback: catalog использует compact horizontal cards и icon-only filter; favorite control унифицирован между catalog/detail через shared disabled `bookmark` button; Home leaf-icon удалён. Fullscreen safe-area provider синхронизирует Telegram `safeAreaInset` и `contentSafeAreaInset`, tokens берут max SDK/CSS/iOS `env()`; `requestFullscreen()` вызывается только при Telegram API >=8.0, старые clients остаются на `expand()` без SDK console error. Floating glass navbar fixed 64px; bottom inset меняет только offset; AppShell main reserve применяется ко всем product routes. Emulation evidence: top=64px, bottom=34px → main reserve=122px, navbar=64px/bottom≈42px, последний catalog card остаётся на ≈32px выше navbar; favorite controls 44×44, одинаковые bookmark/label; web/UI typecheck и targeted lint PASS, console errors 0. User запросил commit/push для production test; real iPhone Telegram fullscreen safe-area всё ещё требует проверки.
...
1. Утвердить scope E2-D2-T07 UI consistency primitives: выполнить как blocker перед E2-D2-T06 done или отдельной следующей задачей.
...
- **От кого / кому:** AI agent → пользователь / следующий агент.
- **Статус задачи:** E2-D2-T06 `review`; implementation complete.
...
- **Next:** approve E2-D2-T07 execution choices, then implement or decide it is separate after E2-D2-T06.
