...
- **Текущая задача:** E7-D8-T03 (`review`) — habit share UI (DEC-064: own-workout N/A).
- **Последняя закрытая задача:** E7-D8-T02 (`done`) — remove + revoke; edge HTTP 32/32.
- **Статус:** T03: GET shares, GET /habits/shared, HabitShareSheet (friend toggles streak/history), detail owner «Поделиться» + shared read-only, rhythm «С вами поделились». typecheck/lint PASS.
- **README sync:** 65 done / 1 blocked / 12 backlog / 0 in_progress / 1 review.
- **Evidence:** friends HTTP scripts; T03 smoke: share habit → friend list → detail → revoke.
- **Следующее точное действие:** user smoke T03; then T04/T05.
...
- **T08 plan/evidence:** `.temp/E4-D5-T08/plan.md`; verification-first matrix выполнена без runtime-изменений. Pure/API/D1/browser/scheduler checks PASS для четырёх schedule types, multiple completions, privacy, skip/rest, icons/colors, DST и idempotence; fixtures удалены. Root typecheck/lint/build/deploy-check PASS, lint diagnostics 0.
- **Следующее точное действие:** выбрать и начать следующую карточку этапа 5. Residual T08: production/real-device Cron и downstream Telegram delivery не входят в локальное evidence; перед production T07 также остаются deploy/real-device scheduler checks.
