...
- **Статус:** approved plan E0-D0-T05 реализуется одним batch: Konsta UI 5.2.0, только `ios`, system iOS typography, Flowly palette; direct Konsta для standard atoms, `packages/ui` оставлен только для approved gaps по DEC-035/037. Общая migration ещё `in_progress`; onboarding S-MA-002–005 и Home S-MA-010 прошли отдельные обязательные Apple HIG/Konsta quality passes внутри той же карточки.
- **Последний подтверждённый результат:** DEC-041 Profile/Settings code+screenshot pass выполнен без Playwright по прямому запрету пользователя. `/help`/S-MA-096 route, HelpScreen/CSS и profile entry удалены; current inventory 68 IDs/96 pairs/68 states. Profile использует full-width Konsta Navbar с icon-only `router.back()`, native List spacing, identity row и future sections disabled с Badge `Скоро`; Settings — тот же Navbar contract и grouped BlockTitle/List/ListInput/Segmented/Toggle/BlockFooter. TimezonePicker вынесен в единый `@/components/timezone-picker` и используется onboarding/settings. Profile/Settings CSS Modules и legacy Select/TextField/Card wrappers удалены. Konsta 5.2.0 ListInput `title:null` crash закрыт documented `title=""` workaround. Web typecheck, targeted lint, production build (20 routes после Help removal), source audits и diff-check PASS. Browser/Playwright validation намеренно не выполнялась; пользователь проверяет screenshots/runtime вручную. Latest screenshot corrections: `Профиль` → `Имя в Flowly`, input switched to direct Konsta outlined ListInput, week title/control gap added, `Оформление` → `Тема оформления` with duplicate inner label removed, neutral autosave footer removed and final List bottom margin collapsed. Targeted typecheck/lint/diff-check PASS. Follow-up screenshot correction: week selector `mt-2`; theme outer List surface removed, then restored by explicit user request; name input List changed to non-inset `mt-8 mb-2` after screenshot showed BlockTitle overlap; theme List surface restored by explicit user request. Focus screenshot confirmed direct Konsta `ListInput`; double ring came from Flowly global `:focus-visible`. По прямому user request global focus rule и its component exceptions removed entirely; Konsta native focus rendering remains. DEC-042: configurable week start removed from onboarding/settings and public `/me`; Monday is fixed and re-auth normalizes DB. Settings bottom gap reduced by removing local `pb-safe-4` and forcing final List `!mb-0`; shared shell nav reserve remains. DEC-043: shared AppShell Navbar now owns exact top-level headers; every main tab has Settings left and avatar/Profile right, Home centers Flowly name with greeting removed, remaining titles match bottom tabs. Typecheck/lint/build/diff-check PASS. Settings persistence audit: direct authenticated PATCH name/timezone works and survives GET, but dev UI intentionally skips mutation; report toggles are local-only and contradict D1 (UI weekly=true, DB weekly/monthly=false, API fields 400); theme is localStorage-only. Values used for HTTP repro restored. User approved DEC-044 fix: dev/prod now share real PATCH autosave; weekly/monthly settings load/save through existing D1 fields; theme remains localStorage-only. Exact authenticated PATCH+GET rerun persisted name/timezone/reports, then canonical values restored. Typecheck, targeted lint, build and diff-check PASS. Local 777001 is temporarily onboarding-complete for current main-page manual review; restore NULL after review. Bottom Tabbar labels set to exact 11px per user request. User requested commit+push of the full current working tree for real-device deployment. Full tree committed as `1a8bf3a` (`feat(web): migrate frontend to Konsta UI`) and pushed to `origin/main`; GitHub Actions Deploy web run 29393979325 completed success in 1m27s, including typecheck, lint, D1 migrations and web worker deploy. New real-device blocker found afterward: fullscreen sticky Navbar collapsed from composed safe-area offset to Konsta `top-0` on scroll. Local fix sets Primary/Profile/Settings root Navbar top to `var(--component-safe-area-top)`; requires checks/deploy and real-device rerun.
...
- Параллельный Home UI follow-up по production Telegram screenshots: Home использует `/me.photoUrl` (`t.me/i/userpic/**`) в 44px circular avatar с `user-round` fallback; direct `.flow-screen` children получили `flex-shrink:0`, screen margin стал horizontal-only, section gap приведён к shared 12px. Home CSS module переименован в `home-screen-v2.module.css`, чтобы Telegram WebView получил новый asset вместо stale cached CSS. Fresh current-HEAD checks 360/390/430: overflow 0, avatar 44×44/radius 50%, все top-level gaps ровно 16px, section slices не сжимаются; typecheck/targeted lint/build PASS; новый CSS asset `1lnn4ocqof_yw.css`.
...
1. Повторно выполнить web/UI/root checks после последних onboarding edits и full repository audits (raw controls/fetch, stale legacy styles/imports/fonts, duplicate safe-area owners).
2. Завершить route/state/browser matrix для guarded product routes с ожиданием реального content, shell persistence, wide viewport, nested top safe-area 59+46 и last-content clearance.
3. Исправить найденные regressions, синхронизировать полное evidence и только после полного PASS перевести E0-D0-T05 в `review`.
...
- **Deploy secrets:** GitHub Actions secrets `CLOUDFLARE_ACCOUNT_ID` и stable `CLOUDFLARE_API_TOKEN` установлены; manual `Deploy web` workflow прошёл успешно. Cloudflare Worker secret `TELEGRAM_BOT_TOKEN` установлен для production/test, но текущий локально найденный source value из `apps/web/.dev.vars` проверен через Telegram `getMe` и невалиден (`401 Unauthorized`) — нужен rotate на реальный BotFather token в Cloudflare Worker settings. `INVIDIOUS_BASE_URL` вынесен в `wrangler.jsonc` vars для production/test (`https://inv.zoomerville.com`). Live YouTube API всё ещё возвращает `cache: unavailable` — нужно отдельно проверить доступность выбранного Invidious instance из Cloudflare Worker или заменить provider instance.
