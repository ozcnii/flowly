...
- **Активная задача:** **E3-D4-T06** (`in_progress`).
- **Статус:** T01–T05 `done`. T04 skip + T05 rest: planned rest (type) vs user `rest` vs `skipped`; shared `postProgramDayTerminal`; progress counters; UI «Сегодня отдыхаю» / «Пропустить».
...
1. **E3-D4-T06** — backend lifecycle occurrences for program days (unique, enrollment/day link, TZ).
2. T07 reminder jobs + DoD §55.3.
