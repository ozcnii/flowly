...
1. **E3-D4-T02** — начало программы: дата старта, enrollment, timezone day mapping (S-MA-052).
2. Then T03 progress → T04 skip → T05 rest → T06 occurrences → T07 jobs+DoD.
3. Prod: ensure remote migration `0011` + seed `0008_starter_programs.sql` on deploy if not auto.
