...
1. E2-D2-T08 закрыта `done` (re-verification PASS, commit+push по явному разрешению пользователя). Следить за production deploy CI: remote migration 0008 (partial unique index) применится автоматически; если упадёт на дублейкатах — безопасно прерывается без partial deploy.
2. Следующий фокус — E2-D3-T02 (step/mixed session): расширяет тот же DEC-062 `/sessions/[id]` runtime, не создавая параллельную session/player архитектуру. Browser-Back contract T01 не менять без exact pre-Home sequence; device-only residuals E0 остаются non-blocking.
3. Visible report UI (text Badge `YouTube`) остаётся E6 (calendar/report screens пока backlog); report-source data contract уже подтверждён через occurrence→workout join.
