...
1. **Optional residual stage 2:** S-MA-032 mixed chooser (video vs steps for `format=mixed` workouts) — not required for DoD.
2. **Stage 3** programs, or other backlog the user picks.
3. **E2-D3-T03 frozen** (DEC-064) until explicit demand.
4. **E6-D7-T09** manual log with calendar (DEC-065). Report Badge `YouTube` remains E6.
