...
- **Активная задача:** E4-D5-T04 — недельная цель и интервальное расписание, `review`.
...
- **Остаточные риски T04:** ambiguous/nonexistent local-time policy для hourly DST не расширялась за approved DEC-068 local-calendar semantics; create flow всё ещё может оставить habit без schedule при ошибке второго запроса.
- **Следующее точное действие:** user/real-device verification S-MA-062; после подтверждения перевести T04 в `done`; occurrences и S-MA-060 progress остаются T07 scope.
